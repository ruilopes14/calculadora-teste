...
pasta_config = os.path.join(os.getenv("APPDATA"), "Calculadora")
os.makedirs(pasta_config, exist_ok=True)
ficheiro_config = os.path.join(pasta_config, "config.json")
...
def abrir_menu():
    global menu
    menu = QMenu(janela)
    from estilos import gerar_estilos
    e = gerar_estilos(tema_atual, caminho_seta, caminho_calen)
    menu.setStyleSheet(e["menu"]) 
    ...
    menu.addAction("Definições", ir_para_defenicoes)
...
def converter_moedas () :
    ...
    if foco_origem: 
        ...
        if moeda_origem == "EUR" : 
            ...
            valor_conversao = round(valor_conversao, casas_decimais)
...
def formatar_numero(numero):
    numero = round(numero, casas_decimais)
...
def carregar_config():
    global casas_decimais, tema_atual
    if os.path.exists(ficheiro_config):
        with open(ficheiro_config) as f:
            config = json.load(f)
        casas_decimais = config.get("casas_decimais", 4)
...
def ir_para_defenicoes () :
    ...
    print(f"Casas decimais: {casas_decimais}")
...
def casas_decimais_ () : 
    global casas_decimais
    casas_decimais = ui.spinBox_casas_decimais.value ()
    print(f"Casas decimais: {casas_decimais}")
    guardar_config()
...
ui.botao_data_atual_3.clicked.connect(lambda: ui.date_edit_2.setDate(QDate.currentDate()))
